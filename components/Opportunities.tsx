...
const stages = [
  { id: 'proposal', label: 'Proposal', color: 'bg-orange-500', probability: 65 },
  { id: 'negotiation', label: 'Negotiation', color: 'bg-purple-500', probability: 80 },
  { id: 'closed-won', label: 'Closed Won', color: 'bg-green-500', probability: 100 },
  { id: 'closed-lost', label: 'Closed Lost', color: 'bg-red-500', probability: 0 }
];
      ...
      <Tabs value={currentView} onValueChange={setCurrentView}>
                      ...
                      <>
                        {opportunity.stage === 'proposal' && (
                          <>
                            ...
                            <Button size="sm" onClick={() => handleMoveToNegotiation(opportunity)} className="bg-purple-600 hover:bg-purple-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          {stages.map((stage) => {
            ...
            const stageValue = stageOpportunities.reduce((sum, opp) => sum + opp.dealValue, 0);
              ...
              <Card key={stage.id} className="min-h-96">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`w-6 h-6 rounded-full ${stage.color} text-white flex items-center justify-center text-xs`}>
                ...
                <CardContent className="p-3 space-y-2">
                  {stageOpportunities.map((opportunity) => (
                    <Card key={opportunity.id} className="p-3 cursor-pointer hover:shadow-sm transition-shadow">
                      <div className="space-y-2">
                        ...
                        <div className="mt-2 space-y-1">
                          {opportunity.stage === 'proposal' && (
                            <>
                              <Button size="sm" variant="outline" onClick={() => handleSendProposal(opportunity)} className="w-full text-xs">
                                <FileText className="h-3 w-3 mr-1" />
                                Send Proposal
                              </Button>
                              <Button size="sm" onClick={() => handleMoveToNegotiation(opportunity)} className="w-full text-xs bg-purple-600 hover:bg-purple-700">
